...
func TestHTTPErrorCode(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			err:      &Error{Code: http.StatusNotFound, Message: "not found"},
			wantCode: http.StatusNotFound,
			wantOk:   true,
		},
		{
			name:     "Error with 200 code",
		...
		{
			name:     "wrapped Error",
			err:      fmt.Errorf("wrapped: %w", &Error{Code: http.StatusForbidden, Message: "forbidden"}),
		...
		{
			name:     "Error with candidates",
			err:      &Error{Code: http.StatusBadRequest, Message: "ambiguous", Candidates: []Candidate{{ModulePath: "example.com/a"}, {ModulePath: "example.com/b"}}},
			wantCode: http.StatusBadRequest,
			wantOk:   true,
		},
	}
